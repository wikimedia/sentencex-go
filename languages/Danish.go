...
func (helper *DaWordContinuityHelper) ContinueInNextWord(textAfterBoundary string) bool {
	...
	return re.MatchString(textAfterBoundary)
}

func (helper *DaWordContinuityHelper) GetLastWord(text string) string {
	words := regexp.MustCompile(`[\s\.]+`).Split(text, -1)
	return words[len(words)-1]
}
func NewDanish() *Danish {
	language := NewLanguage()
	language.Abbreviations = NewSetFromArray(DanishAbbreviations)
	language.IsPunctuationBetweenQuotes = false
	language.WordContinuityHelper = &DaWordContinuityHelper{}
	return &Danish{
		Language: *language,
	}
}
